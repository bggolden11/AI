...
# * Helper function to string values to boolean
def toBoolean(s: str):
    s.strip()
    if s.strip() == "Yes":
        return True
...
def parse(fileName):
    with open(f"TrainingData/{fileName}") as csv_file:
...
# * Build tree given attribute and place into restaurant list
...
# * Calculate and return total positives / (total positives + total negatives)
...
def main(argv):
    if len(sys.argv) < 2:
        print(len(sys.argv))
        print("ERROR: Please input the csv file you would like to use for the model")
        return
    if len(sys.argv) > 2:
        print("ERROR Please only input the csv file you would like to parse")
        return

    # * Parse Data
    restaurants: list[Restuarant] = parse(sys.argv[1])
    ...
    finalRoot.prettyPrintTree(finalRoot)
